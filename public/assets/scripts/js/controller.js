...
function isValidVal($val, $get = 'bool') {
    return (isset($val) && !empty($val) && $get === 'value' ? $val : (isset($val) && !empty($val) ? true : false));
}
...
function empty($val) {
    return ((typeof $val === 'object' && Object.keys($val).length == 0) || (Array.isArray($val) && $val.length == 0));
}
